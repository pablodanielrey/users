...
class ImplicitGrant:
    ...
    auth_url = os.environ['OIDC_HOST'] + '/oauth2/auth'

    def __init__(self, client_id, verify=True):
        self.verify = verify
        self.client_id = client_id
    ...
    def access_token(self, scopes=[]):
        data = {
            'client_id': self.client_id,
            'response_type': 'token',
            #'redirect_uri': 'no es necesaria de acuerdo a la spec',
        ...
        # application/x-www-form-urlencoded
        #r = requests.get(self.auth_url, verify=self.verify, allow_redirects=False, params=params)
        #return r
        ''' en vez de hacer la llamada desde el servidor la hace el cliente mediante una redirección '''
        return self.auth_url + "?" + urllib.parse.urlencode(data)
